---
name: assess
description: Critically double-check your work — code, research, or plans — AFTER doing it. Usually triggered by the Stop hook when a turn ends; `/assess turbo` adds a cross-model panel on top of the self-review. The mirror of this, run BEFORE work starts on the request rather than the result, is the separate `scope` skill.
---
...
Both run AFTER the work. The before-work panel is a **separate skill** (`scope`) sharing
only `panel.sh`: opposite timing, opposite aggregation, and it reviews nothing — so running
it does not satisfy the Stop hook, and this skill still applies when the work is done.
...
3. Run the panel. Pass the task through a **quoted heredoc**, never pasted inline into the
   command — a task containing `$VAR`, backticks or `$(...)` would otherwise be expanded or
   executed by the shell, silently altering the exact wording that is the point of passing
   it verbatim:
   ```
   TASK="$(cat <<'VERBATIM'
   <the original request, pasted exactly as written>
   VERBATIM
   )"
   bash ~/.claude/skills/assess/panel.sh "$ART" "$TASK"
   ```
